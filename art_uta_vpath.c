...
#if 0
static void
art_uta_add_point (ArtUta *uta, int x, int y)
{
  int ix;
  int xt, yt;
  int xf, yf;
  ...
  xt = (x >> ART_UTILE_SHIFT) - uta->x0;
  yt = (y >> ART_UTILE_SHIFT) - uta->y0;
  /* Should this expand the uta? Or should it be better practice to
     compute the bounding box ahead of time? */
  if (xt < 0 || xt >= uta->width || yt < 0 || yt >= uta->height)
    art_die ("uta_add_point: bounds check\n");
  xf = x & (ART_UTILE_SIZE - 1);
  yf = y & (ART_UTILE_SIZE - 1);
  ix = yt * uta->width + xt;
  bb = uta->utiles[ix];
  if (bb == 0)
    bb = ART_UTA_BBOX_CONS(xf, yf, xf + 1, yf + 1);
  else
    bb = ART_UTA_BBOX_CONS(MIN(ART_UTA_BBOX_X0(bb), xf),
		       MIN(ART_UTA_BBOX_Y0(bb), yf),
		       MAX(ART_UTA_BBOX_X1(bb), xf + 1),
		       MAX(ART_UTA_BBOX_Y1(bb), yf + 1));
  uta->utiles[ix] = bb;
}
#endif
